...
<head>
    ...
    <title>Perizinan PJ Gedung — Senior Resident</title>
...
<body>
    ...
    <div class="app-container">
        ...
        <div class="d-flex align-items-center justify-content-between mb-4">
            <div>
                <h5 class="fw-bold mb-0 text-white">Pengajuan Izin PJ</h5>
                <small class="text-slate-400" style="font-size:0.75rem; color:#94a3b8;">Formulir & Status Perizinan PJ</small>
            </div>
            <a href="{{ route('pj.profil') }}">
                <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name ?? 'PJ Gedung') }}&background=6366f1&color=fff&bold=true" class="rounded-circle border border-2 border-indigo shadow-sm" width="38" height="38" alt="Profile">
            </a>
        </div>
        ...
        <div class="card card-custom p-4">
            <h6 class="fw-bold mb-3 d-flex align-items-center gap-2 text-white"><i class="bi bi-file-earmark-plus-fill text-indigo fs-5" style="color:#818cf8;"></i> Form Izin PJ Baru</h6>
            
            <form action="{{ route('pj.perizinan.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-3">
                    ...
                    <select name="jenis_izin" class="form-select rounded-3 py-2" required>
                        ...
                        <option value="Izin Kegiatan">Izin Absen Tugas / Kegiatan</option>
                        <option value="Izin Bermalam">Izin Meninggalkan Asrama</option>
                    </select>
                </div>
                ...
                <div class="row g-2 mb-3">
                    <div class="col-6">
                        <label class="form-label small fw-semibold text-slate-300">Mulai</label>
                        <input type="datetime-local" name="tanggal_mulai" class="form-control rounded-3" required>
                    </div>
                    <div class="col-6">
                        <label class="form-label small fw-semibold text-slate-300">Selesai</label>
                        <input type="datetime-local" name="tanggal_selesai" class="form-control rounded-3" required>
                    </div>
                </div>

                <div class="mb-3">
                    <label class="form-label small fw-semibold text-slate-300">Alasan Utama</label>
                    <textarea name="alasan" class="form-control rounded-3" rows="3" placeholder="Tuliskan keterangan perizinan secara mendetail..." required></textarea>
                </div>

                <div class="mb-4">
                    <label class="form-label small fw-semibold text-slate-300">Upload Dokumen Lampiran (Opsional)</label>
                ...
                <button type="submit" class="btn btn-primary w-100 rounded-3 py-2 fw-semibold shadow-sm" style="background: var(--primary-gradient); border: none;">
                    <i class="bi bi-send-fill me-1"></i> Ajukan Permohonan Izin
                </button>
            </form>
        </div>
        ...
        <div class="d-flex justify-content-between align-items-center mb-3 mt-4">
            <h6 class="fw-bold mb-0 text-white">Riwayat Perizinan PJ</h6>
        ...
        <div class="card card-custom p-3">
            ...
            @empty
                <div class="text-center py-4 text-slate-400">
                    ...
                    <small>Belum ada permohonan izin diajukan.</small>
                </div>
            @endforelse
        </div>
